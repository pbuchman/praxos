# TASK-04: Migrate llm-orchestrator Domain Layer
...
Replace all `SupportedModel` usages with `ResearchModel` in llm-orchestrator domain layer. Replace hardcoded model strings with `LlmModels` constants.

## Files to Modify

### 1. `apps/llm-orchestrator/src/domain/research/models/Research.ts`

**Change import (line ~9):**
```typescript
// FROM:
import { getProviderForModel, type LlmProvider, type SupportedModel } from '@intexuraos/llm-contract';
...
### 2. `apps/llm-orchestrator/src/domain/research/models/index.ts`
...
### 3. `apps/llm-orchestrator/src/domain/research/usecases/submitResearch.ts`
...
### 4. `apps/llm-orchestrator/src/domain/research/usecases/processResearch.ts`
...
### 5. `apps/llm-orchestrator/src/domain/research/usecases/checkLlmCompletion.ts`
...
### 6. `apps/llm-orchestrator/src/domain/research/usecases/enhanceResearch.ts`
...
### 7. `apps/llm-orchestrator/src/domain/research/usecases/retryFailedLlms.ts`
...
### 8. `apps/llm-orchestrator/src/domain/research/usecases/retryFromFailed.ts`
...
```bash
npm run typecheck -w @intexuraos/llm-orchestrator
```
...
- [ ] Typecheck passes for llm-orchestrator
