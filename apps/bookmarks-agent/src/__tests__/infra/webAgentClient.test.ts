...
describe('webAgentClient', () => {
  ...
  describe('default logger', () => {
    ...
    it('uses default logger when none provided', async () => {
      delete process.env['LOG_LEVEL'];
      const clientWithoutLogger = createWebAgentClient({
        baseUrl,
        internalAuthToken,
      });
      ...
      const result = await clientWithoutLogger.fetchPreview('https://example.com/');

      expect(result.ok).toBe(true);
    });

    it('uses custom log level from env when available', async () => {
      process.env['LOG_LEVEL'] = 'debug';
      const clientWithDebugLevel = createWebAgentClient({
        baseUrl,
        internalAuthToken,
      });
      ...
      const result = await clientWithDebugLevel.fetchPreview('https://example.com/');
