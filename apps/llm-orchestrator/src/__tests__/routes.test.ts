...
describe('Research Routes - Authenticated', () => {
  ...
  describe('POST /research/draft', () => {
    it('creates draft with Google API key (title generation success)', async () => {
      const token = await createToken(TEST_USER_ID);
      ...
      if (saved !== undefined) {
        ...
        // Title should be generated by fake title generator
    ...
    it('creates draft with Google API key but title generation fails (fallback)', async () => {
      ...
      const newServices: ServiceContainer = {
        ...
        createResearchProvider: (_model, _apiKey, _userId, _pricing, _logger) => createFakeLlmResearchProvider(),
        createSynthesizer: (_model, _apiKey, _userId, _pricing, _logger) => createFakeSynthesizer(),
        createTitleGenerator: (_model, _apiKey, _userId, _pricing, _logger) => failingTitleGenerator,
        createContextInferrer: (_model, _apiKey, _userId, _pricing, _logger) => createFakeContextInferrer(),
        createInputValidator: (_model, _apiKey, _userId, _pricing, _logger) => createFakeInputValidator(),
      };
      ...
      try {
        ...
        if (saved !== undefined) {
          expect(saved.status).toBe('draft');
          // Should fall back to first 60 chars of prompt
  ...
  describe('PATCH /research/:id', () => {
    ...
    it('returns 500 when findById fails (repository error)', async () => {
      const token = await createToken(TEST_USER_ID);
      fakeRepo.setFailNextFind(true);
  ...
  describe('POST /research/:id/approve', () => {
    ...
    it('rejects draft with no models and no contexts', async () => {
      const token = await createToken(TEST_USER_ID);
      const research = createTestResearch({
        status: 'draft',
        selectedModels: [],
      ...
      expect(body.error.message).toContain('Select at least one model or provide input context');
    });
  });
  ...
  describe('POST /research/validate-input', () => {
    ...
    it('does not call improvement when quality is GOOD even with includeImprovement=true', async () => {
      const token = await createToken(TEST_USER_ID);
      fakeUserServiceClient.setApiKeys(TEST_USER_ID, { google: 'test-google-key' });
      ...
      const goodValidator: InputValidationProvider = {
        async validateInput(_prompt: string) {
          return ok({
            quality: 2,
            reason: 'Good quality prompt',
            usage: { inputTokens: 0, outputTokens: 0, costUsd: 0 },
          });
        },
        async improveInput(_prompt: string) {
          // This should NOT be called when quality is 2
          throw new Error('improveInput should not be called for GOOD quality');
        },
      };
      ...
      const newServices: ServiceContainer = {
        ...
        createTitleGenerator: (_model, _apiKey, _userId, _pricing, _logger) => createFakeTitleGenerator(),
        createContextInferrer: (_model, _apiKey, _userId, _pricing, _logger) => createFakeContextInferrer(),
        createInputValidator: (_model, _apiKey, _userId, _pricing, _logger) => goodValidator,
      };
      setServices(newServices);
      ...
      try {
        ...
        expect(body.success).toBe(true);
        expect(body.data.quality).toBe(2);
        expect(body.data.reason).toBe('Good quality prompt');
