/**
 * Real-time bookmark changes listener with cost optimizations.
 *
...
export interface UseBookmarkChangesResult {
...
export function useBookmarkChanges(enabled = true): UseBookmarkChangesResult {
  const { getAccessToken, user, isAuthenticated } = useAuth();
  const [changedBookmarkIds, setChangedBookmarkIds] = useState<string[]>([]);
  ...
  const setupListener = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      // 💰 CostGuard: Listener only reads minimal metadata (userId, updatedAt, ogFetchStatus)
      // Full bookmark data fetched separately via API
