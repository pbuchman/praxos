...
describe('WhatsAppClient', () => {
  ...
  describe('markAsReadWithTyping', () => {
      ...
      nock(GRAPH_API_BASE)
        .post(`/v22.0/${PHONE_NUMBER_ID}/messages`, (body: object) => {
          ...
          return (
            ...
            b.typing_indicator?.type === 'text'
          );
        })
        .reply(200, { success: true });

      const client = createWhatsAppClient(config);
      const result = await client.markAsReadWithTyping(MESSAGE_ID);
    ...
    it('returns error on network failure', async () => {
      ...
      if (!result.ok) {
        expect(result.error.code).toBe('NETWORK_ERROR');
        expect(result.error.message).toContain('Failed to mark message as read with typing');
      }
    });
    ...
    it('returns timeout error when request takes too long', async () => {
      ...
      const client = createWhatsAppClient(config);
      const resultPromise = client.markAsReadWithTyping(MESSAGE_ID);
