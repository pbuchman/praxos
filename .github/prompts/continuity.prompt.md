...
2. Number issue files deterministically:
   ===
   0-0-[title].md  
   0-1-[title].md  
   1-0-[title].md  
...
4. After each subtask:
  - Move its status from *Now* to *Done*.
  - Update *Next* with the upcoming task.
...
- Code or command snippets must always use === markers.
- Standard code fences (```), backticks, or nested blocks are FORBIDDEN.
